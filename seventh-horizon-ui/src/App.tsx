...
function AppRefactored() {
  ...
  const tagsCol = useMemo(
    () => header.findIndex((h: any) => String(h ?? '').toLowerCase() === 'tags'),
    [header],
  );
  ...
  const filteredBody = useMemo(() => {
    ...
    return searchedBody.filter((row: any[]) => {
  ...
  const hourCounts = useMemo(() => {
    const idxUTC = header.findIndex((h: any) => String(h ?? '').toLowerCase() === 'utc');
  ...
  const handleSort = useCallback(
    (colIdx: number) => {
      const existing = state.sortKeys.find((k: SortKey) => k.idx === colIdx);
      let next: SortKey[];
